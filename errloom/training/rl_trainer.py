# TODO certain structures we created need to be compatible with the transformers package Trainer class. They use Dict for everything (LOLLLL) so as long as we support data[key] and data.get(...) access we're probably good
# TODO all the broadcast_data sent to accelerate - can we create data structures for those as well? we can to_dict() at the end. We could create several types of BroadcastData types depending on if there are multiple uses
# TODO the logs are kinda ugly and hard to follow
# TODO maybe implement SPPO
# TODO we should have the ability to submit a dataset subsection and run that, not passing a huge full dataset
# TODO we should try to not make this depend on loom, instead the loom calls the trainer
...
class RLAlgorithm(ABC):
    ...
    def __init__(self, config: RLConfig, logger: EnhancedLogger):
        self.config = config
        self.logger = logger
...
class BatchRows:
    ...
    TODO this is a crappy data structure, overly specified to q/a type problems. We want each loom rollout to receive the full row data with each dataset column to operate on it to its liking.
    """
    prompts: List[Any]
    answers: List[Any]
    tasks: List[Any]
...
class RepeatSampler(Sampler):
    ...
    # 1. ensures each row is repeated across multiple processes. This guarantees that identical prompts are
...
class ProcessedBatchData:
    ...
    rewards: List[float]
    all_gravities_dict: Dict[str, Any] = field(default_factory=dict)
...
class RLTrainer(Trainer):
    def __init__(
        ...
        self.dry = dry
        self.algorithm = algorithm or GRPOAlgorithm(config=args, logger=self.logger)
        self.tokenizer = tokenizer
        self.pipe = TrainingPipelineFactory.create_default(
            gradient_accumulation_steps=args.gradient_accumulation_steps,
            num_iterations=args.num_accum
        )
        self.async_generator = AsyncBatchGenerator(loom=self.loom, generation_timeout=args.async_generation_timeout)
        self.model_accepts_loss_kwargs = False
        self.num_batches_ahead = args.num_batches_ahead  # 0 will behave synchronously (submit and wait immediately)
        ...
        # TODO we should be able to pass in a client to reuse one, since we might do nested RLTrainers
        # TODO we should move this elsewhere, this simply initializes a vllm_client and connects to vllm_server
        if not self.dry:
            ...
            self._step_synced = 0  # Initialize to 0 since vLLM already has initial weights
        else:
    ...
    def _enable_gradient_checkpointing(self, model: PreTrainedModel, args: RLConfig) -> PreTrainedModel:
    ...
    def _inner_training_loop(self, *args, **kwargs):
        ...
        try:
            # TODO maybe we could start_async_generator() here instead of _prepare_inputs
            return super()._inner_training_loop(*args, **kwargs)
        finally:
            self.stop_async_generator()
    ...
    def _ids_to_tensors(self,
        ...
        ids = [context_ids[i] + completion_ids[i] for i in range(len(context_ids))]
        mask = [context_mask[i] + completion_mask[i] for i in range(len(context_mask))]
    ...
    def _prepare_inputs(  # type: ignore
        ...
        if self.pipe.orchestrator.should_generate_new_batch(self._step, self._buffered_inputs):
            self.generate()

        step_inputs = self.pipe.input_buffer_strategy.get_step_inputs(self, self._step)
    ...
    def generate(self):
        self.logger.push_info("GEN", f"[bold blue]🔄 STEP {self.state.global_step}[/] [dim]→[/] Generating completions...")
        if self.pipe.weight_sync_strategy.should_sync_weights(self.state.global_step, self._step_synced):
            self.logger.info(f"[yellow]⚡ Syncing weights[/] to vLLM (step {self.state.global_step})")
            self.sync_weights()

        self.start_async_generator()
        self.accelerator.wait_for_everyone()

        # Plan generation phase using orchestrator
        phase = self.pipe.orchestrator.plan_generation_phase(self._step, self.async_generator)
        batches_submitted = 0  # TODO this is never incremented..?
        for bId in range(self._next_batch_id, phase.target_batch_id + 1):
            self.logger.push_info(str(bId))
            bOffset = bId - phase.retrieve_batch_id
            bRows = self.pipe.batch_data_strategy.gather_batch_data(self, bOffset)

            localBatchSize = len(bRows.prompts) // self.accelerator.num_processes

            # Submit batch (main process only)
            if self.accelerator.is_main_process:
                bId = self.state.global_step

                # TODO logger.info for each line
                self.logger.info(f"  [cyan]📦 Batch {bId}[/] [dim]│[/] "
                                 f"prompts=[green]{len(bRows.prompts)}[/] "
                                 f"processes=[blue]{self.accelerator.num_processes}[/] "
                                 f"local_size=[yellow]{localBatchSize}[/]")
                ...
                request = BatchRequest(
                    ...
                    rows=(Dataset.from_dict({
                        'prompt': bRows.prompts,
                        'text':   bRows.prompts,
                        'answer': bRows.answers,
                        'task':   bRows.tasks,
                        'info':   bRows.infos
                    })),
                    ...
                    processing_config=(ProcessingConfig(
                        tokenizer_class=self.processing_class,  # type: ignore
                        mask_env_responses=self.mask_env_responses,
                    )),
                ...
                self.logger.info(f"    [green]✓ Submitted[/] batch {bId} to async generator")
                self.logger.pop()
            self.accelerator.wait_for_everyone()
        ...
        if self.accelerator.is_main_process:
            self.logger.info(f"[bold purple]📤 RETRIEVAL[/] [dim]→[/] Batch {phase.retrieve_batch_id}")

            # Get batch result
            batch = self.async_generator.get_batch(phase.retrieve_batch_id)
            broadcast_data = {
                'prompt_ids':         batch.processed_results['prompt_ids'],
                'prompt_mask':        batch.processed_results['prompt_mask'],
                'completion_ids':     batch.processed_results['completion_ids'],
                'completion_mask':    batch.processed_results['completion_mask'],
                'main_gravity':            batch.processed_results.get('main_gravity') or batch.gravities.main_gravity,
                'all_gravities_dict': batch.all_gravities_dict if hasattr(batch, 'all_gravities_dict') else {'reward': batch.processed_results.get('main_gravity') or batch.gravities.main_gravity},
                'completions':        batch.completions if hasattr(batch, 'completions') else [],
                'prompts':            batch.prompts if hasattr(batch, 'prompts') else [],
            }
        else:
            broadcast_data = None
        ...
        rollout_rewards = broadcast_data['main_gravity']
        ...
        all_advantages = self.algorithm.compute_advantages(all_rewards, self.num_generations)
        ...
        if self.accelerator.is_main_process:
            self._log_reward_metrics_primary("train", broadcast_data['all_gravities_dict'], all_rewards, len(rollout_rewards))
            self._log_textual_data_primary(broadcast_data['prompts'], broadcast_data['completions'], broadcast_data['all_gravities_dict'])
        ...
        context_ids_padded = pad_sequence(prompt_ids_list, batch_first=True, padding_value=pad_id)
        context_mask_padded = pad_sequence(prompt_mask_list, batch_first=True, padding_value=0)
        completion_ids_padded = pad_sequence(completion_ids_list, batch_first=True, padding_value=pad_id)
        ...
        context_ids = torch.cat([context_ids_padded, completion_ids_padded], dim=1)
        context_mask = torch.cat([context_mask_padded, completion_mask_padded], dim=1)
        processed_inputs = TrainingInputs(
            input_ids=context_ids,
            attention_mask=context_mask,
            prompt_ids=context_ids_padded,
            prompt_mask=context_mask_padded,
        ...
        self._buffered_inputs = self.pipe.input_buffer_strategy.buffer_inputs(self, processed_inputs)
    ...
    def compute_loss(self,
        ...
        importance_ratios = self.algorithm.compute_importance_ratios(per_token_logps, old_per_token_logps, completion_mask)
        lossc = self.algorithm.calculate_policy_loss(importance_ratios, advantages, completion_mask)
        self.logger.pop()

        # Compute the KL divergence between the model and the reference model
        # TODO what is this for? why do we want to do kl divergence between a ref model and the trained model
        if self.beta != 0.0:
            with torch.no_grad():
                if self.ref_model is not None:
            ...
            if hasattr(lossc, 'loss'):
                # For algorithms that return token-level losses, add KL to each token
                if len(per_token_kl_loss.shape) == 2:  # token-level
                    loss = lossc.loss + (per_token_kl_loss * completion_mask).sum() / completion_mask.sum()
                else:  # sequence-level
                    loss = lossc.loss + per_token_kl_loss.mean()
            else:
                loss = lossc.loss + per_token_kl_loss.mean()
        ...
        metrics = self._metrics.get_mode_metrics(mode)
        alg_name = self.algorithm.get_algorithm_name()

        for k, v in lossc.metrics.reward_metrics.items():
            if isinstance(v, torch.Tensor):
                v = v.item()
            metrics.algorithm.add_algorithm_metric(alg_name, k, v)

        metrics.clipping.add_clipping_stats(lossc.clipping_stats, self.accelerator)

        self.logger.pop()
        return loss

    def evaluate(self, eval_dataset=None, ignore_keys=None, metric_key_prefix="eval", **kwargs) -> Dict[str, Any]:
        """
        ...
        if 'main_gravity' in tapestry:
            rewards = torch.tensor(tapestry['main_gravity'])
        ...
        if 'completion' in tapestry:
            completions = tapestry['completion']
        ...
        if self.accelerator.is_main_process and self.log_completions and 'prompt' in tapestry:
            # Prepare textual logs
            prompts = tapestry['prompt'][:self.num_completions_to_print]
            completions = tapestry['completion'][:self.num_completions_to_print]

            # Extract rewards for logging
            reward_dict = {}
            if 'reward' in tapestry:
                reward_dict['reward'] = tapestry['reward'][:self.num_completions_to_print]
            for key in tapestry:
                if key.startswith('reward_') and key != 'reward':
                    reward_dict[key] = tapestry[key][:self.num_completions_to_print]
    ...
    def log(self,
        ...
        # TODO what is this log function doing
    ...
    def _log_reward_metrics_primary(
        self,
        mode: str,
        all_gravities_dict: Dict[str, Any],
        ...
        for reward_key in all_gravities_dict:
            if reward_key != 'reward':  # Skip the consolidated reward
                reward_values = all_gravities_dict[reward_key]
                if isinstance(reward_values, list):
    ...
    def _log_textual_data_primary(
        ...
        """
        self._textual_logs.add_logs(all_prompts, all_completions, all_gravities_dict)
...
def create_gspo_trainer(
    loom: Loom,
    model: PreTrainedModel,
    args: RLConfig,  # Can reuse GRPOConfig, GSPO uses different epsilon values
    tokenizer: PreTrainedTokenizerBase,
    callbacks: Optional[list[TrainerCallback]] = None,
    optimizers: Optional[OptimizerConfig] = None,
    peft_config: Optional[PeftConfig] = None,
...
class GenerationPhase:
    ...
    retrieve_batch_id: int
    target_batch_id: int
    submit_batch_ids: List[int]
...
class BatchDataStrategy(ABC):
    ...
    @abstractmethod
    def gather_batch_data(self, trainer: 'RLTrainer', batch_offset: int) -> BatchRows:
...
# TODO do we really need to keep them separate like that? why not just one TrainingMetrics that we clear, fill up, and clone as we go? This would make sense perhaps if we're running eval() mid train()
@dataclass
class ModeMetrics:
...
class DefaultTrainingOrchestrator(TrainingOrchestrator):
    ...
    def plan_generation_phase(self, step: int, async_generator: AsyncBatchGenerator) -> GenerationPhase:
        ...
        return GenerationPhase(
            ...
            retrieve_batch_id=batch_id_to_retrieve,
            target_batch_id=target_batch_id,
            submit_batch_ids=list(range(batch_id_to_retrieve, target_batch_id + 1))
        )
...
class VLLMWeightSyncStrategy(WeightSyncStrategy):
    ...
    def sync_weights(self, trainer: 'RLTrainer') -> None:
        ...
        # Debug log
        trainer.logger.info(f"Process {trainer.accelerator.process_index}: Starting weight sync to vLLM")

        # ALL processes must participate in model operations for DeepSpeed ZeRO-3
        if is_peft_model(trainer.model):
            # With PEFT and DeepSpeed ZeRO Stage 3, we must gather the full model at once before merging
            with gather_if_zero3(list(trainer.model.parameters())):  # type: ignore
                trainer.model.merge_adapter()  # type: ignore

                # Update vLLM weights while parameters are gathered
                for name, param in trainer.model.named_parameters():  # type: ignore
                    # When using PEFT, we need to recover the original parameter name and discard some parameters
                    name = name.removeprefix("base_model.model.").replace(".base_layer", "")
                    if trainer.model.prefix in name:  # type: ignore
                        continue
                    ...
                    if trainer.accelerator.is_main_process:
                        trainer.vllm_client.update_named_param(name, param.data)

                trainer.model.unmerge_adapter()  # type: ignore
        else:
        ...
        if trainer.accelerator.is_main_process:
            trainer.vllm_client.reset_prefix_cache()
...
class DefaultBatchDataStrategy(BatchDataStrategy):
    ...
    def gather_batch_data(self, trainer: 'RLTrainer', batch_offset: int) -> BatchRows:
        ...
        batches = trainer._async_dataloader.peek_ahead(batch_offset)  # TODO what type is this, what is 'batches'
...
class DefaultBatchProcessingStrategy(BatchProcessingStrategy):
    ...
    def process_batch_result(self,
        ...
        broadcast_data = {
            ...
            'all_gravities_dict': batch_result.all_gravities_dict if hasattr(batch_result, 'all_gravities_dict') else {'reward': processed_results.get('rewards', [])},
        ...
        # Create rewards tensor and compute advantages using full batch
        all_rewards = torch.tensor(broadcast_data['rewards'], device=trainer.accelerator.device)
        all_advantages = trainer.algorithm.compute_advantages(all_rewards, trainer.num_generations)
        # Now create tensors only for this process's slice
        prompt_ids_list = []
        prompt_mask_list = []
        completion_ids_list = []
        completion_mask_list = []
        for i in range(process_slice.start, process_slice.stop):
            prompt_ids_list.append(torch.tensor(broadcast_data['prompt_ids'][i], device=trainer.accelerator.device))
            prompt_mask_list.append(torch.tensor(broadcast_data['prompt_mask'][i], device=trainer.accelerator.device))
            completion_ids_list.append(torch.tensor(broadcast_data['completion_ids'][i], device=trainer.accelerator.device))
            completion_mask_list.append(torch.tensor(broadcast_data['completion_mask'][i], device=trainer.accelerator.device))

        # Pad sequences
        pad_token_id = getattr(trainer.processing_class, 'pad_token_id', 0)
        ...
        # Truncate if needed
        if trainer.max_prompt_length is not None and prompt_ids.size(1) > trainer.max_prompt_length:
            prompt_ids = prompt_ids[:, -trainer.max_prompt_length:]
            prompt_mask = prompt_mask[:, -trainer.max_prompt_length:]
        if trainer.max_completion_length is not None and completion_ids.size(1) > trainer.max_completion_length:
            completion_ids = completion_ids[:, :trainer.max_completion_length]
            completion_mask = completion_mask[:, :trainer.max_completion_length]
...
class DefaultInputBufferStrategy(InputBufferStrategy):
    ...
    def buffer_inputs(self, trainer: 'RLTrainer', processed_inputs: TrainingInputs) -> List[TrainingInputs]:
    ...
    def get_step_inputs(self, trainer: 'RLTrainer', step: int) -> TrainingInputs:
