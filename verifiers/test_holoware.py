...
from verifiers.holoware.spans import RoleSpan, TextSpan, ContextResetSpan, ClassSpan, ObjSpan, SamplerSpan
...
The capital of France is Paris."""
    ...
    assert isinstance(spans[1], RoleSpan)
    assert spans[1].role == 'system'

    assert isinstance(spans[2], TextSpan)
    assert "helpful assistant" in spans[2].text

    assert isinstance(spans[3], RoleSpan)
    assert spans[3].role == 'user'

    assert isinstance(spans[4], TextSpan)
    assert "capital of France" in spans[4].text

    assert isinstance(spans[5], RoleSpan)
    assert spans[5].role == 'assistant'
...
"""
    ...
    assert isinstance(spans[0], ContextResetSpan)
    assert spans[0].train is True
    assert isinstance(spans[1], RoleSpan) and spans[1].role == 'system'
    assert isinstance(spans[2], TextSpan)
    assert isinstance(spans[3], RoleSpan) and spans[3].role == 'user'
    ...
    assert isinstance(spans[6], RoleSpan) and spans[6].role == 'assistant'
    assert isinstance(spans[7], SamplerSpan) and spans[7].id == 'compressed' and spans[7].fence == 'compress'
    
    # Second conversation
    assert isinstance(spans[8], ContextResetSpan)
    assert spans[8].train is True
    assert isinstance(spans[9], RoleSpan) and spans[9].role == 'system'
    assert isinstance(spans[10], TextSpan)
    assert isinstance(spans[11], RoleSpan) and spans[11].role == 'user'
    assert isinstance(spans[12], TextSpan)
    assert isinstance(spans[13], ObjSpan) and spans[13].var_ids == ['compressed']
    assert isinstance(spans[14], RoleSpan) and spans[14].role == 'assistant'
    assert isinstance(spans[15], SamplerSpan) and spans[15].id == 'decompressed' and spans[15].fence == 'decompress'

    # Third conversation
    assert isinstance(spans[16], ContextResetSpan)
    assert spans[16].train is False
    assert isinstance(spans[17], RoleSpan) and spans[17].role == 'system'
    assert isinstance(spans[18], TextSpan)
    assert isinstance(spans[19], RoleSpan) and spans[19].role == 'user'
    ...
    assert len(spans[23].var_args) == 1
    assert "Compare the two objects" in spans[23].var_args[0]
    assert isinstance(spans[24], ClassSpan) and spans[24].class_name == 'FidelityCritique'
    assert isinstance(spans[25], RoleSpan) and spans[25].role == 'assistant'
    assert isinstance(spans[26], SamplerSpan) and spans[26].fence == 'think'
    assert isinstance(spans[27], SamplerSpan) and spans[27].fence == 'json'
    assert isinstance(spans[28], ClassSpan) and spans[28].class_name == 'FidelityAttractor'
    assert spans[28].var_args == ['original', 'decompressed']
...
"""
    ...
    assert len(spans) == 4
    assert isinstance(spans[0], RoleSpan) and spans[0].role == 'user'
    assert isinstance(spans[1], TextSpan) and "Hello user" in spans[1].text
    assert isinstance(spans[2], RoleSpan) and spans[2].role == 'assistant'
...
def test_sampler_span_parsing():
    ...
    assert len(spans) == 2
    assert isinstance(spans[0], RoleSpan) and spans[0].role == 'assistant'
    assert isinstance(spans[1], SamplerSpan)
    assert spans[1].id == 'sampler_id'
...
This is outside the block."""
    ...
    assert isinstance(container_span.body, holoware.Holoware)
    
    body_spans = container_span.body.spans
    
    # The body is "    This is the body.\n    <|@_@|>\n    Inner assistant message."
    # After dedenting and parsing, an implicit system role should be created.
    assert len(body_spans) == 4
    assert isinstance(body_spans[0], RoleSpan) and body_spans[0].role == 'system'
    assert isinstance(body_spans[1], TextSpan) and body_spans[1].text.strip() == "This is the body."
    assert isinstance(body_spans[2], RoleSpan) and body_spans[2].role == 'assistant'
